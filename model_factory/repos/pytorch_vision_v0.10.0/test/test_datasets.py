    ...
    def inject_fake_data(self, tmpdir, config):
        ...
        return dict(
            num_examples=num_images_per_file
                         * len(
                self._VERSION_CONFIG["train_files"] if config["train"] else self._VERSION_CONFIG["test_files"]),
...
class OmniglotTestCase(datasets_utils.ImageDatasetTestCase):
    ...
    def inject_fake_data(self, tmpdir, config):
        target_folder = (
                pathlib.Path(
                    tmpdir) / "omniglot-py" / f"images_{'background' if config['background'] else 'evaluation'}"
        )
